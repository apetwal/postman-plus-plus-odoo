...
class ApiConnector(models.Model):
    ...
    def send_request(self, event_record=None):
        _logger.info("Sending a request.")
        PARAMS = {}
        HEADERS = {}
        ...
        _logger.info(f'Request URL {r.url}')
        ...
        if r.status_code == 200:
            try:
                ...
                _logger.debug(f"Response Fetched {self.response}")
    ...
    def _get_request_headers(self, event_record):
        _logger.info("Fetching Request Headers")
        headers_dict = {}
        for request in self:
        ...
        _logger.debug(f"Request Headers fetched {headers_dict}")
        return headers_dict

    def _get_request_parameters(self, event_record):
        _logger.info("Fetching Request Parameters")
        parameters_dict = {}
        for request in self:
        ...
        _logger.debug(f"Request Parameters fetched {parameters_dict}")
        return parameters_dict

    def _add_to_url(self, event_record):
        _logger.info("Adding Custom Parameters to URL")
        ...
        _logger.debug(f"New URL to Call {url_to_call}")
        return url_to_call
    ...
    def _fetch_key_value_pair_response(self):
        _logger.info("Fetching Key Value pair for response handler")
        actual_values = {}
        ...
        _logger.debug(f"Key Value pair fetched for response handling {actual_values}")
        return actual_values

    def open_oauth_user_authentication_url(self):
        _logger.info("Starting OAuth2 Authentication Method")
    ...
    def _get_oauth_authorization_url(self):
        _logger.info("Fetching OAUTH2 Authorization URL")
    ...
    def trigger_response(self):
        _logger.info("Response Handler Triggered")
        converted_key_value = self._fetch_key_value_pair_response()
        if self.state == 'object_create':
            _logger.info("Creating a new Record")
            self.env[self.target_model_name].create(
                converted_key_value
            )
            _logger.debug(f"Model in which record is created {self.target_model_name}")
            _logger.debug(f"New Record Details {converted_key_value}")
        elif self.state == 'object_write':
            _logger.info("Updating an existing Record")
            set_string = ""
            ...
            _logger.debug(f"Running database query: {query_string}")
    ...
    @staticmethod
    def _flatten_dict(d):
        _logger.info("Flattening a dictionary")
        ans = {}
        for k, v in d.items():
