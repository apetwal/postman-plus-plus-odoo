...
class ImportPostmanCollection(models.Model):
        ...
        if self._is_Base64(self.json_file_for_import) is not False:
            ...
            try:
                ...
                for individual_request in postman_collection_json["item"]:
                    ...
                    _logger.info('Creating new api.connector record')
                    _logger.debug(f"Record Details {converted_api_connector_record}")
                ...
                _logger.info("Postman Collection Imported")
            except JSONDecodeError:
                raise UserError("Malformed JSON File!!")
            except ValueError:
                raise UserError("A postman API with same name already exists")
            except KeyError:
                raise UserError("Invalid POSTMAN Collection.")
        else:
    ...
    def _create_request_header(self, header_array, api_connector_id):
        ...
        for header in header_array:
            ...
            _logger.info('Creating new api.header record')
            _logger.debug(f"Record Details {header_dict}")
    ...
    def _create_request_parameter(self, request, api_connector_id):
        ...
        if isinstance(request["url"], dict):
            for parameters in request["url"]["query"]:
                ...
                _logger.info('Creating new api.parameter record')
                _logger.debug(f"Record Details {parameter_dict}")
